...
gr_dates<-gtd %>% select(gname, imonth, iyear)  %>% mutate(date=paste(imonth,"01",iyear, sep="/"))  %>% mutate(date=as.Date(date, "%m/%d/%Y")) %>% group_by(gname) %>% summarise(first_moyr=min(date), last_moyr=max(date)) %>% ungroup 
...
gr_dates <- as.list(as.data.frame(t(gr_dates)))
mydata<-do.call(rbind,lapply(gr_dates[12:14],mk_data))
